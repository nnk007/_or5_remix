...
export default function Projects({ items }: { items: { name: string, description: string, tech: string[], tasks: string[], demo_a: string, img_src: string }[], }) {
    ...
    return (
        <div className={`grid grid-flow-row sm:grid-flow-col grid-rows-[50%_50%] sm:grid-cols-[50%_50%] sm:grid-rows-1 w-full sm:h-screen overflow-hidden transition-[grid-template-columns] ease-out duration-300`} style={projectActive != null ? { gridTemplateColumns: items.map((i, k) => { return k == projectActive ? '90%' : '10%' }).join(' ') } : {}}>
...
function Project({ item, collapsed, active, onClick: handleClick }: { item: { name: string, description: string, tech: string[], tasks: string[], demo_a: string, img_src: string }, collapsed: boolean, active: boolean, onClick: () => void }) {
    ...
    return (
        <div className='w-full h-screen relative transition-all duration-300 cursor-pointer group'
            onClick={() => {
                handleClick();
            }}
        >
            <div className={`transition-all z-10 absolute text-white items-center justify-center grid ${active ? 'grid-cols-[0%_100%] sm:grid-cols-[50%_50%]' : 'grid-cols-[100%_0%] sm:grid-cols-[100%]'} grid-rows-[100%]  h-full w-full grid-flow-col max-h-full items-center justify-items-center ${collapsed ? 'invisible' : ''}`}>
                <div className={`flex flex-col h-[8ch] w-5/6 drop-shadow-[0_0_2px_#000] transition-all ${collapsed ? 'sm:hidden' : ''}`}>
